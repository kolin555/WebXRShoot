...
namespace WebXR.Interactions
{
  public class ShootingController : MonoBehaviour
  {
    ...
    private void Update()
    {
      ...
      if (controller.GetAxis(WebXRController.AxisTypes.Trigger) > 0.5f&& Gun.canShoot)
      {
        controller.Pulse(0.3f,100);
        GunShoot();
      }
      if (controller.GetAxis(WebXRController.AxisTypes.Trigger) < 0.3f&& controller.GetAxis(WebXRController.AxisTypes.Trigger)>0.1f&& !Gun.canShoot)
      {
        GunTigeerReturn();
      }
      /*
      if (controller.GetButton(WebXRController.ButtonTypes.Trigger))
      {
      ...
      */
      ...
      if (hasAnimator)
      {
        animator.Play(animationStateName, -1, 1);
      }
    }
    ...
    public void LockGun(Rigidbody gun)
    {
      gun.MovePosition(transform.position);
      attachJoint.connectedBody = gun;
    }

    public void GunShoot()
    {
      
      Gun.GetCommond(GunActionType.Shoot);
    }

    public void GunTigeerReturn()
    {
      Gun.GetCommond(GunActionType.GunTriggerReturn);
    }
